...
真实 Telegram Bot API 每秒最多推送约 30 条消息。本项目的 Load Demo 在 1 秒内向管道灌入 1200 条消息——是真实场景吞吐量的 40 倍。这不是模拟"正常使用"，这是对处理管道的**压力测试**。

| 维度 | 模拟压力（本项目） | 真实 Telegram |
|------|-------------------|---------------|
| 吞吐上限 | 1200 条/秒 | ~30 条/秒 |
| 测试目标 | 管道在极限流量下是否一致 | 管道在正常流量下是否可靠 |
| 状态一致性压力 | 极高（50 群并发写入） | 低（稀疏到达） |
| 可复现性 | 完全可复现（一键 demo） | 不可控（依赖外部服务） |
| 评审者体验 | 打开即演示，不需要任何配置 | 需注册 Bot、配 Token、设 Webhook |

**真实场景的"脏"不在吞吐量，在正确性：**

真实 Telegram 接入会引入几类问题——但它们跟管道的处理能力是正交的：

- **消息重复投递** —— Telegram 可能在超时后重试同一条消息。本项目用 `message_id` 幂等去重覆盖了这个问题（SET NX 原子操作）。
- **连接不稳定** —— Webhook 超时、长轮询断连。本项目用三段式降级（healthy → degraded → local）覆盖了"下游存储不可用"的情况。
- **消息乱序到达** —— 本项目用 per-group `asyncio.Lock` 保证同群串行处理，不同群完全并行。

**管道本身（validate → dedup → classify → state machine → Redis → SSE）跟消息来源无关。** 无论是 HTTP POST 模拟、Telegram Webhook、Kafka consumer 还是文件读取，管道的处理逻辑完全一致。

用模拟流量还有一个实际好处：真实 Telegram Bot 无法产生 1200 条的并发涌入，也无法在 Dashboard 上点一个按钮就断掉 Redis——这些刻意设计的破坏性场景，只有模拟能做，而它们恰恰是证明系统边界最有效的方式。
